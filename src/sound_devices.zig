...
pub fn drawSoundGroup(state: *SoundState, config: DrawConfig) i32 {
   ...
   _ = rg.guiLabel(.{
       ...
       .y = @floatFromInt(current_y),
       .width = @floatFromInt(config.label_width),
   ...
   const in_res = rg.guiDropdownBox(
       input_dropdown_bounds,
       "Analogue 1+2;USB Device 1;Default Input",
       &state.capture_device_index,
       state.input_dropdown_open
   );
   if (in_res == 1) {
   ...
   const out_res = rg.guiDropdownBox(
       output_dropdown_bounds,
       "Default Output;Speakers;Headphones",
       &state.playback_device_index,
       state.output_dropdown_open
   );
   if (out_res == 1) {
       state.output_dropdown_open = !state.output_dropdown_open;
   }

   return current_y + config.element_height * 2 + config.margin;
}
